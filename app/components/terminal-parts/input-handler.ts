...
// Commands that should be executed in local bash (not sent to agent)
const BASH_COMMANDS = new Set([
  // Custom commands
  "about", "install", "github",
  // Sandbox commands (prevent sending to agent)
  "diff", "apply", "reset",
  // Common external commands
  "cat", "ls", "grep", "head", "tail", "wc", "sort", "uniq", "tr", "cut",
  "sed", "awk", "find", "xargs", "tee", "diff", "patch", "mkdir", "rmdir",
  "rm", "cp", "mv", "touch", "chmod", "chown", "ln", "basename", "dirname",
  "realpath", "date", "sleep", "seq", "yes", "env", "which", "whoami",
  "hostname", "uname", "curl", "wget", "tree", "jq",
  // Bash builtins
  ".", ":", "[", "alias", "bg", "break", "builtin", "caller", "cd", "command",
...
export function createInputHandler(term: Terminal, bash: Bash) {
  ...
  // Determine if a command should run in local bash or be sent to the agent
  const isLocalCommand = (input: string): boolean => {
    // "!command" prefix forces local bash execution
    if (input.startsWith("!")) return true;
    // "agent ..." prefix forces agent (handled elsewhere)
    if (input.startsWith("agent ")) return false;
    // Pipelines, redirections, variable assignments, subshells → local bash
    if (/[|><;`$]/.test(input) || /^[A-Za-z_]\w*=/.test(input)) return true;
    // Check first word against known commands
    const firstWord = input.split(/\s+/)[0];
    return BASH_COMMANDS.has(firstWord);
  };
  ...
  const executeCommand = async (command: string) => {
    ...
    if (trimmed === "clear") {
    ...
    } else if (isLocalCommand(trimmed)) {
      // Strip "!" prefix if used for forced local execution
      const execCmd = trimmed.startsWith("!") ? trimmed.slice(1) : trimmed;
      const result = await bash.exec(execCmd);
      if (result.stdout)
        term.write(
      ...
      // Strip "agent " prefix if explicitly used
      const prompt = trimmed.startsWith("agent ") ? trimmed.slice(6) : trimmed;
      const result = await bash.exec(`agent ${prompt}`);
      if (result.stdout)
        term.write(
          formatMarkdown(colorizeUrls(result.stdout)).replace(/\n/g, "\r\n")
        );
