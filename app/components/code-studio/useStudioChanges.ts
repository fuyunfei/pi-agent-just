...
export function useStudioChanges() {
	...
	const mountPointRef = useRef(mountPoint);

	// Keep refs in sync for downloadFiles
	...
	const handleAction = useCallback(
		async (action: "download" | "clear") => {
			if (action === "download") {
				downloadFiles(changesRef.current, mountPointRef.current);
				return;
			}
...
/** Bundle all in-memory files into a zip and trigger download (uses state, no extra fetch). */
function downloadFiles(changes: OverlayChange[], mountPoint: string) {
...
function createZip(files: { name: string; content: string }[]): Uint8Array {
	...
	for (const entry of entries) {
		...
		cv.setUint32(16, hv.getUint32(14, true), true);
		cv.setUint32(20, hv.getUint32(18, true), true);
		cv.setUint32(24, hv.getUint32(22, true), true);
